...
public class IssueUpsertedConsumer {
  ...
  public IssueUpsertedConsumer(KafkaConfig kafkaConfig, ReportingService reportingService, ReactiveRedisTemplate<String, ReportingResultDto> cache, KafkaTemplate<String, ReportingResultDto> kafkaTemplate) {
  ...
  public void subscribe() {
    ...
    ReceiverOptions<String, String> options = kafkaConfig
        .baseReceiverOptions(GROUP_ID)
    ...
    this.subscription = receiver.receive()
      .concatMap(record -> {
        try {
          logger.debug("📥 Received message from topic: {}, partition: {}, offset: {}, key: {}", 
                      record.topic(), record.partition(), record.offset(), record.key());
          
          IssueUpsertedEvent event = mapper.readValue(record.value(), IssueUpsertedEvent.class);
          logger.info("✅ Parsed issue event: {}", event);
          
          return processIssueEvent(event)
              .doOnSuccess(v -> logger.debug("✅ Successfully processed issue: {}", event.getIssueKey()))
              .doOnError(error -> logger.error("❌ Failed to process issue: {}", event.getIssueKey(), error))
              .then(record.receiverOffset().commit())
              .doOnSuccess(v -> logger.debug("✅ Committed offset for issue: {}", event.getIssueKey()));
              
        } catch (Exception e) {
          logger.error("❌ Failed to parse message from topic: {}, key: {}", record.topic(), record.key(), e);
  ...
  private Mono<Void> processIssueEvent(IssueUpsertedEvent event) {
    return reportingService                              // 1. Recalculate aggregates
            ...
            .flatMap(report ->                          // 2. Cache the new report and propagate it
                    cache.opsForValue()
                         ...
                         .then(Mono.fromFuture(
                                 kafkaTemplate.send("report.monthly.updated", event.getProjectKey(), report)
                         ))
                         .thenReturn(report)
            )
            .then()                       // convert to Mono<Void> at the end for simpler generics
            .onErrorResume(ex -> {                      // 3. Never block the consumer
