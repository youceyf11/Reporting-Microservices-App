...
class JiraClientTest {
    ...
    class FetchProjectIssuesTests {
        ...
        void fetchProjectIssues_shouldHandleEmptyResponse() {
            ...
            // Act & Assert
            StepVerifier.create(jiraClient.fetchProjectIssues("EMPTY-PROJ", 50))
                    .verifyComplete();
        }
        ...
        void fetchProjectIssues_shouldHandleNotFound() {
            ...
            mockWebServer.enqueue(new MockResponse()
                    .setResponseCode(404)
                    .setBody("Project not found"));

            // Act & Assert - JiraClient has onErrorResume that returns empty Flux
            StepVerifier.create(jiraClient.fetchProjectIssues("NONEXISTENT", 100))
                    .verifyComplete();
        }
    ...
    class FetchIssuesByJqlTests {
        ...
        void fetchIssuesByJql_shouldHandleInvalidJql() {
            ...
            mockWebServer.enqueue(new MockResponse()
                    .setResponseCode(400)
                    .setBody("Invalid JQL syntax"));

            // Act & Assert - JiraClient has onErrorResume that returns empty Flux
            StepVerifier.create(jiraClient.fetchIssuesByJql("INVALID JQL SYNTAX", 100))
                    .verifyComplete();
        }
        ...
        void fetchIssuesByJql_shouldEncodeJqlCorrectly() throws InterruptedException {
            ...
            // Act
            StepVerifier.create(jiraClient.fetchIssuesByJql(jqlWithSpecialChars, 100))
                    .verifyComplete();
    ...
    class PerformanceAndReliabilityTests {
        ...
        void shouldHandleConcurrentRequests() {
            ...
            // Act & Assert
            StepVerifier.create(
                    jiraClient.fetchProjectIssues("PROJ-1", 100)
