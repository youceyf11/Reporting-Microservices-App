...
public class ExcelReportService {
    ...
    public ByteArrayResource generateEmployeeReport(String projectKey) {
        Mono<ReportingResultDto> monthlyReportMono = reportingClient.get()
        ...
        Mono<Map<String, Map<Integer, Double>>> weeklyStatsMono = reportingClient.get()
                .uri(uriBuilder -> uriBuilder
    ...
    private XSSFWorkbook buildWorkbook(ReportingResultDto report,
                                       Map<String, Map<Integer, Double>> weeklyStats) {

        List<EmployeePerformanceDto> rows = Optional.ofNullable(report.getEmployeeRankings())
                .orElseGet(List::of);
        ...
        String[] header = {
                "Employee",
                "Week-1 (h)", "Week-2 (h)", "Week-3 (h)", "Week-4 (h)",
                "MonthHours (h)",
                "Efficiency (%)",
                "PerformanceLevel",
                "AvgResolution (h)",
