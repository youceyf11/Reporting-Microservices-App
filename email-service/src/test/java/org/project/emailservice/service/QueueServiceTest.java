...
class QueueServiceTest {
    ...
    void queueEmail_sends_to_high_routing_key() {
        EmailRequest req = EmailRequest.builder()
                .priority(EmailPriority.HIGH).build();
        ...
        StepVerifier.create(queueService.queueEmail(req, "id1"))
                    .verifyComplete();

        verify(rabbitTemplate).convertAndSend(eq("email.exchange"),
                                              eq("email.high"),
                                              /* payload JSON */ anyString());
    }
}