 ...
 *
 * <p>Cette configuration crée les beans Sender et Receiver nécessaires pour une communication 100%
 * non-bloquante avec RabbitMQ.
 *
 * @author Votre nom
 * @since 1.0
 */
  ...
  public void ensureQueuesExist() {
    ...
    try {
      ...
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(URGENT_PRIORITY_QUEUE_NAME, true))
              .to(emailExchange)
              .with("email.urgent"));
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(HIGH_PRIORITY_QUEUE_NAME, true))
              .to(emailExchange)
              .with("email.high"));
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(NORMAL_QUEUE_NAME, true))
              .to(emailExchange)
              .with("email.normal"));
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(LOW_PRIORITY_QUEUE_NAME, true))
              .to(emailExchange)
              .with("email.low"));
      ...
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(URGENT_DLQ, true))
              .to(new TopicExchange(DLX_NAME))
              .with(URGENT_PRIORITY_ROUTING_KEY));
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(HIGH_DLQ, true))
              .to(new TopicExchange(DLX_NAME))
              .with(HIGH_PRIORITY_ROUTING_KEY));
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(NORMAL_DLQ, true))
              .to(new TopicExchange(DLX_NAME))
              .with(NORMAL_PRIORITY_ROUTING_KEY));
      amqpAdmin.declareBinding(
          BindingBuilder.bind(new Queue(LOW_DLQ, true))
              .to(new TopicExchange(DLX_NAME))
              .with(LOW_PRIORITY_ROUTING_KEY));
