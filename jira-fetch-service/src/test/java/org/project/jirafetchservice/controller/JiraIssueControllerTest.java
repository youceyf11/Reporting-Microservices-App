...
class JiraIssueControllerTest {
    ...
    void getIssue_shouldReturnIssue_whenValidKey() {
        // Arrange
        when(jiraIssueService.getIssue("PROJ-123"))
                .thenReturn(Mono.just(mockJiraResponse));
    ...
    void getProjectKeyFromIssue_shouldReturnProjectKey() {
        // Arrange
        when(jiraIssueService.getProjectKeyFromIssue("PROJ-123"))
                .thenReturn(Mono.just("PROJ"));
    ...
    void getProjectIssues_shouldReturnIssues() {
        ...
        when(jiraIssueService.getProjectIssues("PROJ"))
                .thenReturn(Flux.just(mockJiraResponse));
        when(jiraMapper.toSimpleDtoFromApi(mockJiraResponse))
                .thenReturn(mockSimpleDto);
    ...
    void getProjectIssues_shouldRespectLimit() {
        ...
        when(jiraMapper.toSimpleDtoFromApi(any()))
                .thenReturn(mockSimpleDto);
    ...
    void searchIssues_shouldReturnSearchResults() {
        ...
        when(jiraIssueService.searchIssues(jql))
                .thenReturn(Flux.just(mockJiraResponse));
    ...
    void getIssuesAssignedTo_shouldReturnAssignedIssues() {
        ...
        when(jiraIssueService.getIssuesAssignedTo(email))
                .thenReturn(Flux.just(mockJiraResponse));
    ...
    void syncSearch_shouldReturnSyncedResults() {
        // Arrange
        String jql = "project = PROJ";
        when(jiraIssueService.synchroniserSearchAvecJira(jql))
                .thenReturn(Flux.just(mockSimpleDto));
    ...
    void getLocalIssue_shouldReturnLocalIssue() {
        // Arrange
        when(jiraIssueService.recupererIssueLocale("PROJ-123"))
                .thenReturn(Mono.just(mockSimpleDto));
    ...
    void validation_shouldReturn400_whenEmptyPathVariable() {
        ...
        webTestClient.get()
                ...
                .exchange()
                .expectStatus().isBadRequest(); // This triggers validation error for empty issueKey
    }
    ...
    void performance_shouldHandleLargeResultSetWithLimit() {
        // Arrange
        Flux<JiraIssueApiResponse> largeFlux = Flux.range(1, 1000)
                .map(i -> {
