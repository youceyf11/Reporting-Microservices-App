  ...
  public Mono<Void> publish(IssueUpsertedEvent event) {
    ...
    .flatMap(record -> 
      sender.send(Mono.just(record))
        .doOnNext(result -> 
          logger.info("✅ Successfully published issue event: {} to partition: {}", 
                     event.getIssueKey(), result.recordMetadata().partition()))
        .doOnError(error -> 
          logger.error("❌ Failed to publish issue event: {}", event.getIssueKey(), error))
        .then()
    )
    .onErrorResume(error -> {
      logger.error("💥 Critical error publishing issue event: {}", event.getIssueKey(), error);
